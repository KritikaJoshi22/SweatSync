...
function ShowNFT() {
  const [nftModal] = useGlobalState('nftModal');
//   const [connectedAccount] = useGlobalState('connectedAccount');
  ...
  return (
    <div className={`fixed top-0 left-0 w-full h-screen flex justify-center 
      ...
      transition-transform duration-300 ${nftModal}`}>
      <div className='flex flex-col bg-[#151c25] shadow-xl shadow-[#59a6a6] rounded-xl p-6 w-4/5 md:w-3/6'>
        <div className='flex justify-between items-center'>
          <p className='font-semibold text-gray-200'>Buy NFT</p>
          <button
            type="button"
            onClick={() => setGlobalState('nftModal', 'scale-0')}>
